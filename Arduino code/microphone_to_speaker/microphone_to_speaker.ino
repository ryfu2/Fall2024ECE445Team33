...
void playSilence() {
    const int silenceBufferLen = bufferLen; // 与正常缓冲区长度一致
    int16_t silenceBuffer[silenceBufferLen] = {0}; // 创建静音缓冲区，所有值为 0
    size_t bytesWritten = 0;

    // 写入几次静音缓冲区，确保所有 DMA 缓冲区都清空
    for (int i = 0; i < 3; i++) {
...
void parseWavHeader(File file) {
    ...
    file.seek(22); // 跳到第 22 字节
    uint8_t numChannels[2];
    file.read(numChannels, 2); // 读取 2 字节
